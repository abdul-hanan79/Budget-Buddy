...
import { bookNameType, productItemType, productType } from "@/types/types";
import axiosWithCookies from "@/services/axios";
import { deleteBookApi, fetchBooksApi, submitBookApi } from "@/services/book";
...
export const deleteRating = createAsyncThunk(
  ...
  async (itemDetails: any) => {
    try {
      const id = itemDetails.id;
      const result = await axios.delete(
        `http://localhost:8080/ratings/deleteRating?id=${id}`
      );
...
export const submitBook = createAsyncThunk(
  "book/submitBook",
  async (data: bookNameType) => {
    try {
      const bookData: bookNameType = data;
      const result = await submitBookApi(bookData);
      console.log("uploaded product", result);
      const bookDetails = result.data;
      console.log("product data is", bookDetails);
      return bookDetails;
    } catch (e) {
...
const bookSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addCase(submitBook.fulfilled, (state, action) => {
      console.log("book details in extraReducers", action.payload);
      let newState: any = {
        ...state,
        books: [...state.books, action.payload.result],
      };
      console.log("new state", newState);
      return newState;
    }),
      builder.addCase(fetchBooks.fulfilled, (state, action) => {
        console.log("books details", action.payload);
        let newState = {
          ...state,
          books: action.payload.result,
        };
        console.log("newState in fetchBooks", newState);
        return newState;
      }),
      builder.addCase(deleteBook.fulfilled, (state, action) => {
        console.log("action.payload", action.payload);
        if (action.payload.message == "successfull") {
          let deletedBook = action.payload.bookId;
          let books: any = state.books;
          let filteredBooks = books.filter(
            (book: any) => book.id !== deletedBook
          );
          let newState: any = {
            ...state,
            books: filteredBooks,
          };
          console.log("new state in deleted book", newState);
          return newState;
        }
        return state;
      }),
        ...
        if (action.payload.message == "successfull") {
          ...
          let filteredProducts = products.map((product: productItemType) => {
            if (product.id == action.payload.productId) {
              return {
                ...product,
                ratings: product.ratings?.filter(
                  (rating: any) => rating.id !== deletedRatingId
                ),
              };
            } else {
              return product;
