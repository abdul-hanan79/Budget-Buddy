...
export const fetchCurrentUser = createAsyncThunk(
  ...
  async () => {
    try {
      ...
      // const result = await axiosWithToken.get(
      //   "http://localhost:8080/user/fetchCurrentUser",
      //   { params: { value } }
      // );
...
export const signupUser = createAsyncThunk(
  "authUser/signupUser",
  async (user: signupUserType) => {
    try {
      console.log("signupUser /authi slice", user);
      const signupUser = await registerApi(user);
...
export const loginUser = createAsyncThunk(
  "auth/loginUser",
  async (userCredentials: loginUserType) => {
    console.log("the logined user", userCredentials);
    try {
      const userDetails: loginUserType = {
        email: userCredentials.email,
        password: userCredentials.password,
      };
      const user = await loginApi(userDetails);
...
export const verifyOTP = createAsyncThunk(
  ...
  async (userDetails: any) => {
    // console.log("the logined user", userCredentials);
    try {
      const user = await verifyOTPAPI(userDetails);
      console.log("logined user data is", user);
      const userData = user.data;
      localStorage.setItem("token", userData.token);
      console.log("UserData", userData);
